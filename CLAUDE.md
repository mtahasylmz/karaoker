...
| code mutation | mutmut; `[tool.mutmut] paths_to_mutate` = pure modules only (`core.py`, `repair.py`, `chunks.py`, `lines.py`, `vad.py`, `ass.py`) — never `backends.py`/`cli.py` (subprocess/GPU wrappers; unit tests can't kill those mutants). Runner must exec pytest DIRECTLY (`python -m pytest`), never `uv run pytest`: mutmut's timeout kills only the runner process, an orphaned pytest child keeps the stdout pipe open, and every infinite-loop mutant then stalls the sweep (untested pile-up + a live mutant left on disk). Runner carries `--timeout=15 --timeout-method=signal` (pytest-timeout): SIGALRM reaps an infinite-loop mutant IN-PROCESS — on macOS mutmut's own subprocess kill can leave the pytest child spinning and wedge the serial sweep, and a `SIGKILL` to escape it leaves a mutant applied on disk. If a sweep ever wedges: `git checkout` the mutated pure module + `rm annemusic/*.py.bak` before re-running |
